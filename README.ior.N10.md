...
* `testFile` - path to data files to be read or written for this
  benchmark

* `hintsFileName` - path to MPI-IO hints file

Providing an MPI-IO "hints" file for the MPI-IO runs, which IOR will
...
be at least 960 GB.  Assuming `blockSize=1MB` and `numTasks=240` (24
MPI processes per node) is optimal, an appropriate `segmentCount`
would be
