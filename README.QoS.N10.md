...
## QoS Resource Division Scaling Test:
    ...
    1) IOR, sequential, N to N, reads and writes
        ...
        c. Rerun the test with this QoS setting and record the read
          and write bandwidth in the output.

        d. For a job count of 2, 3, 4, and 5. Run independent jobs
          like 1a, above, simulteneously where the read and write
          bandwidth QoS for each job is set to the same 20% value used
          for 1c.  Record read and write bandwidth for each job.
...
<br>
	...
	d.  For a job count of 2, 3, 4, and 5. Run independent jobs
          like 2a, above, simulteneously where the read and write IOPs
          QoS for each job is set to the same 20% value used for 2c.
          Record read and write IOPs for each job.
